...
export function LessonList({ lessons }: { lessons: { lesson_date: string; title: string; material_link: string | null }[] }) {
  ...
  return (
    <div className="space-y-4 pt-2">
      <h1 className="text-xl font-bold text-[#1A1A1A]">{t('portalLessons')}</h1>
      <div className="space-y-3">
        {lessons.length === 0 ? (
          ...
          <div key={i} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
            ...
            <p className="text-sm font-semibold text-[#1A1A1A]">{l.title}</p>
            {l.material_link && (
              <a href={l.material_link} target="_blank" rel="noopener noreferrer"
                className="text-xs text-[#F5B800] mt-1 inline-block hover:underline">
