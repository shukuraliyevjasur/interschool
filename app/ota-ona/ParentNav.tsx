...
export function ParentNav() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-30 bg-[#F5B800] h-16 flex items-center px-4">
        ...
        {links.map(l => (
          <Link key={l.href} href={l.href}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition-colors ${
              pathname.startsWith(l.href) ? 'bg-white text-[#F5B800]' : 'text-white/70 hover:text-white hover:bg-white/15'
            }`}>
            {l.label}
          </Link>
        ))}
