        ...
        <div className="grid gap-4 md:grid-cols-2">
          {groups.map((g: any) => (
            <Link key={g.id} href={`/ustoz/davomat?group=${g.id}&date=${today}`}
              ...
              <p className="font-bold text-[#1A1A1A] group-hover:text-[#F5B800] transition-colors">{g.name}</p>
              <p className="text-xs text-gray-400 mt-1">
                {g.schedule_days?.join(', ') || '—'} {g.schedule_time ? `· ${g.schedule_time}` : ''}
              </p>
              <p className="text-xs text-[#F5B800] mt-3 font-medium">Bugungi davomat belgilash &rarr;</p>
            </Link>
          ))}
        </div>
      )}
