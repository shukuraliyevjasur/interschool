    ...
    <>
      <div className="mb-5 flex flex-wrap gap-3">
        ...
        <button
          ...
          className="shrink-0 px-5 py-2 rounded-xl bg-amber-500 text-white text-sm font-semibold hover:bg-amber-600 transition-colors"
        >
              ...
              <tbody>
                {coverage.map(c => (
                  <tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    <td className="px-5 py-3 font-mono text-sm text-[#1A1A1A]">
                      {c.date}
                      {c.date === today && <span className="ml-2 px-1.5 py-0.5 rounded text-[10px] bg-amber-100 text-amber-700 font-semibold">bugun</span>}
                    </td>
                    <td className="px-5 py-3 text-[#1A1A1A]">{groupMap[c.group_id] ?? `#${c.group_id}`}</td>
                    <td className="px-5 py-3 text-[#1A1A1A]">{teacherMap[c.teacher_id] ?? `#${c.teacher_id}`}</td>
        ...
        <Modal title="O'qituvchi PIN" onClose={() => setNewPin(null)}>
          <div className="space-y-4">
            ...
            <div className="bg-gray-50 rounded-xl p-4 text-center">
              <p className="text-3xl font-mono font-bold tracking-widest text-[#1A1A1A]">{newPin}</p>
            </div>
            <button
            ...
            <button
              ...
              className="w-full py-2.5 rounded-xl bg-[#F5B800] text-[#1A1A1A] text-sm font-semibold hover:bg-[#D4970A] transition-colors"
            >
              Tushundim
            </button>
          </div>
              ...
              {groups.length === 0 ? (
                ...
                return (
                  <label key={g.id} className="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-gray-50 cursor-pointer">
                    ...
                    />
                    <span className="text-sm text-[#1A1A1A]">{g.name}</span>
                  </label>
                );
              })}
            </div>
