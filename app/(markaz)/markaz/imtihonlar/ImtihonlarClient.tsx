    ...
    <div className="flex flex-col lg:flex-row gap-6">
      ...
      <div className="w-full lg:w-72 lg:flex-shrink-0">
        <button onClick={() => setShowAdd(true)}
          className="w-full mb-4 px-4 py-2.5 rounded-xl bg-[#F5B800] text-[#1A1A1A] text-sm font-semibold hover:bg-[#D4970A] transition-colors">
          + Imtihon qo&apos;shish
        </button>
          ...
          {exams.length === 0 ? (
            ...
            <div key={ex.id}
              className={`p-4 rounded-2xl border cursor-pointer transition-colors ${
                selectedExamId === ex.id ? 'border-[#F5B800] bg-red-50' : 'border-gray-100 bg-white hover:border-gray-200'
              }`}
              onClick={() => router.push(`/markaz/imtihonlar?exam=${ex.id}`)}>
              <p className="font-semibold text-sm text-[#1A1A1A]">{ex.title}</p>
      ...
      <div className="flex-1">
        {!selectedExam ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <div className="mb-5">
              <h2 className="text-lg font-bold text-[#1A1A1A]">{selectedExam.title}</h2>
            ...
            {examStudents.length === 0 ? (
              ...
              <form onSubmit={handleResults}>
                  ...
                  <tbody>
                    {examStudents.map(s => (
                      <tr key={s.id} className="border-b border-gray-50">
                        <td className="py-2 pr-4 font-medium text-[#1A1A1A]">{s.full_name}</td>
                        <td className="py-2 pr-4">
                          <input type="number" name={`score_${s.id}`} defaultValue={examResults[s.id]?.score ?? ''}
                            min={0} max={selectedExam.max_score} placeholder="—"
                            className="w-20 px-2 py-1 rounded-lg border border-gray-200 text-sm focus:outline-none focus:border-[#F5B800]" />
                        </td>
                        <td className="py-2">
                          <input name={`note_${s.id}`} defaultValue={examResults[s.id]?.notes ?? ''}
                            className="w-full px-2 py-1 rounded-lg border border-gray-200 text-sm focus:outline-none focus:border-[#F5B800]" />
                        </td>
                      </tr>
                    ))}
                  </tbody>
                ...
                <button type="submit" disabled={isPending}
                  className="px-5 py-2.5 rounded-xl bg-[#F5B800] text-[#1A1A1A] text-sm font-semibold hover:bg-[#D4970A] transition-colors disabled:opacity-50">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowAdd(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-bold text-[#1A1A1A]">Yangi imtihon</h2>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                ...
                <input name="title" required className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#F5B800]" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">Sana</label>
                <input name="exam_date" type="date" required className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#F5B800]" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">Max ball</label>
                <input name="max_score" type="number" defaultValue="100" className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#F5B800]" />
              </div>
              <button type="submit" disabled={isPending}
