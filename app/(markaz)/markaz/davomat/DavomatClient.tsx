...
import { submitAttendance, saveLessonTopic } from './actions';

type Group = { id: number; name: string };
type Student = { id: number; full_name: string };

export function DavomatClient({
  ...
  function navigate(group?: number, date?: string) {
    ...
    router.push(`/markaz/davomat?group=${g}&date=${d}`);
  }
  ...
  function handleTopicSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    const fd = new FormData(e.currentTarget);
    startTransition(async () => { await saveLessonTopic(fd); });
  }
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <input
          ...
          onChange={e => navigate(undefined, e.target.value)}
          className="px-4 py-2 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#F5B800]"
        />
        {students.length > 0 && (
      ...
      {!selectedGroupId ? (
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-4">
            {students.map((s, i) => (
              <div key={s.id} className={`flex items-center gap-4 px-5 py-3 ${i > 0 ? 'border-t border-gray-50' : ''}`}>
                ...
                <span className="flex-1 text-sm font-medium text-[#1A1A1A]">{s.full_name}</span>
          ...
          <button type="submit" disabled={isPending}
            className="px-6 py-2.5 rounded-xl bg-[#F5B800] text-[#1A1A1A] text-sm font-semibold hover:bg-[#D4970A] transition-colors disabled:opacity-50">
      ...
      {selectedGroupId && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <form onSubmit={handleTopicSubmit} className="space-y-3">
            ...
            <input type="hidden" name="date" value={selectedDate} />
            <input
              name="topic" defaultValue={lessonTopic?.title ?? ''} placeholder="Bugungi dars mavzusi..."
              className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#F5B800]"
            />
            <input
              name="material_link" defaultValue={lessonTopic?.material_link ?? ''} placeholder="Material linki (ixtiyoriy)"
              className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#F5B800]"
            />
            <button type="submit" disabled={isPending}
