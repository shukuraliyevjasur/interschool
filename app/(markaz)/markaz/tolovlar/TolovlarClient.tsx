    ...
    <>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          { label: 'Jami yozuvlar', value: String(payments.length), color: 'text-[#1A1A1A]' },
        ].map(c => (
            ...
            <tbody>
              {filtered.map(p => (
                <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3 font-medium text-[#1A1A1A]">{p.students?.full_name ?? '—'}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowAdd(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-bold text-[#1A1A1A]">To&apos;lov qo&apos;shish</h2>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                ...
                <input name="month_label" required placeholder="Iyun 2026" className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#F5B800]" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">Miqdor (so&apos;m)</label>
                <input name="amount_uzs" type="number" required placeholder="500000" className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#F5B800]" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">Muddati</label>
                <input name="due_date" type="date" required className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#F5B800]" />
              </div>
              <div>
              ...
              <div>
                ...
                <input name="notes" className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#F5B800]" />
              </div>
