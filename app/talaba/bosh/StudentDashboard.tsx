  ...
  return (
    <div className="space-y-5 pt-2">
      <div>
        ...
        <h1 className="text-xl font-bold text-[#1A1A1A] mt-0.5">{studentName}</h1>
      </div>
      ...
      {recentAttendance.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/talaba/davomat" className="text-xs text-[#F5B800]">Barchasi &rarr;</Link>
          </div>
      ...
      {recentLessons.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/talaba/darslar" className="text-xs text-[#F5B800]">Barchasi &rarr;</Link>
          </div>
          <div className="space-y-2">
            {recentLessons.map((l, i) => (
              <div key={i}>
                <p className="text-[10px] text-gray-400">{l.lesson_date}</p>
                <p className="text-sm font-medium text-[#1A1A1A]">{l.title}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <div className="space-y-2">
            {examResults.map((r, i) => (
              <div key={i} className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-[#1A1A1A]">{r.exams?.title}</p>
                  <p className="text-[10px] text-gray-400">{r.exams?.exam_date}</p>
                </div>
                <span className="text-sm font-bold text-[#F5B800]">
