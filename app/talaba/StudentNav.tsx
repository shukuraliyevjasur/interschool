...
export function StudentNav() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-30 bg-[#1A1A1A] h-16 flex items-center px-4">
      <span className="text-white font-semibold tracking-widest text-lg font-[family-name:var(--font-barlow)] mr-6">INTER SCHOOL</span>
      <div className="flex gap-1 flex-1 overflow-x-auto">
        {links.map(l => (
          <Link key={l.href} href={l.href}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition-colors ${
              pathname.startsWith(l.href) ? 'bg-white text-[#1A1A1A]' : 'text-white/60 hover:text-white hover:bg-white/10'
            }`}>
            {l.label}
          </Link>
        ))}
